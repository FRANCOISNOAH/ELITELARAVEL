<form method="POST" action="{{route('lecteur.add')}}" name="lecteur" id="lecteur">
    @csrf
    <div class="modal-body">
        <input type="hidden" name="lecteur" value="{{ $operation->id }}"/>
        <table class="table table-sm table-hover mt-3" id='table'>
            ...
            <tbody>
            @foreach($lecteurs as $item)
                <tr>
                    <td><input type="hidden" name="operation" value="{{ $operation->id }}"/></td>
                    <td id='{{$item->id}}'><input type='checkbox' name='lecteurs[]'
